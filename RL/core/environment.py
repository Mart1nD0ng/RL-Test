...
class CoreEnv:
    ...
    def step(self, deltaE: DeltaE, partitions: Dict[int, int]) -> Tuple[Dict[int, Observation], float, Dict]:
        ...
        # 4. 边密度惩罚 (保持稀疏性) — QUADRATIC to break panic-add cycle
        # Linear penalty was too weak: at 1700 edges, penalty = 0.8*(11.3-1) = 8.24
        # Quadratic: at 1700 edges, penalty = 0.3*(11.3-1)^2 = 31.8 — much stronger
        # At 300 edges (2x target), penalty = 0.3*(2-1)^2 = 0.3 — gentle
        e_target = max(1.0, 1.5 * float(n))
        edge_ratio = float(e) / e_target
        edge_excess = max(0.0, edge_ratio - 1.0)
        edge_penalty = edge_excess * edge_excess  # Quadratic scaling
        ...
        reward = (
            ...
            + 0.3 * failure_signal
            # 惩罚项 — quadratic edge penalty sits OUTSIDE the multiplier
            # so even disconnected networks feel the cost of excess edges
            - 0.5 * long_edge_cost
            - 0.1 * edit_cost
            # 延迟惩罚
            - 0.6 * latency_cost
        ...
        # Quadratic edge penalty applied outside connectivity_multiplier
        # This ensures the penalty dominates when edges >> target, breaking
        # the panic-add positive feedback loop
        reward -= 0.3 * edge_penalty
    ...
    def _consensus_success(self) -> float:
        ...
        # 4. Smooth Quorum Proximity Factor (replaces hard threshold)
        #
        # Old behavior: hard cliff at lcc_size == quorum_min
        #   LCC=67 → score ~0.4,  LCC=66 → score ~0.00001  (no gradient!)
        #
        # New behavior: sigmoid centered at quorum_min
        #   LCC >> quorum → factor ≈ 1.0 (full credit)
        #   LCC == quorum → factor = 0.5 (half credit — "almost there")
        #   LCC << quorum → factor → 0   (smooth decay, not a cliff)
        #
        # This gives the Critic a continuous gradient signal:
        # "you're at 60/67 quorum, adding 7 more nodes to LCC will double your score"
        quorum_ratio = lcc_size / max(1, quorum_min)
        _k_sigmoid = 8.0  # Steepness: controls how sharp the transition is
        quorum_factor = 1.0 / (1.0 + math.exp(-_k_sigmoid * (quorum_ratio - 1.0)))

        # 5. Evaluate Quorum Internal Quality (Sampled Path Reliability)
        # Always compute this, even for sub-quorum LCC — the sigmoid handles the penalty.
        # For tiny LCCs (< 2 nodes), skip sampling — reliability is undefined.
        if lcc_size < 2:
            return quorum_factor * (lcc_size / N) * 0.01  # tiny but smooth
        ...
        # 6. Final Score: reliability × size coverage × quorum proximity
        size_ratio = lcc_size / N

        return avg_reliability * size_ratio * quorum_factor
    ...
    def set_physics_override(self, key: str, value: float):
        ...
        This allows the training loop to gradually ramp up difficulty parameters
        like INTERF_K without reloading the config or rebuilding the environment.
        """
