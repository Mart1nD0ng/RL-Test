...
DEFAULT_PHYSICS = {
    # ========== Legacy Model Parameters (backward compat) ==========
    'D0': 200.0,                # Reference distance (m)
    'MSG_BITS': 4000.0,         # Message size (bits)
    'GAMMA': 0.0,               # Poisson spatial density (0 = disable theory latency)
    'EPS_SUCCESS': 1e-12,       # Epsilon for -log(success)
    'P_NI': 0.2,                # Base noise+interference (legacy)
    'INTERF_K': 0.0,            # [LEGACY] Global interference coeff. (only used in legacy path)
    'INTERF_K_LOCAL': 0.3,      # Local congestion interference coeff. N_eff(v) = N_thermal * (1 + λ * deg(v))
    'PT': 1.0,                  # Legacy TX power (unitless)
    'ALPHA': 1.7,               # Legacy path-loss exponent
    'Z': 4.0,                   # Legacy fading parameter
    ...
    # --- Frequency & Geometry ---
    'FC_GHZ': 28.0,             # Carrier frequency (GHz), mmWave band
    'H_UT': 1.5,                # User terminal height (m)
    'H_BS': 10.0,               # Base station / anchor node height (m)

    # --- 3GPP UMi Path Loss Coefficients (TR 38.901 Table 7.4.1-1) ---
    # LOS: PL = PL_CONST_LOS + PL_COEF_DIST_LOS*log10(d3D) + PL_COEF_FREQ_LOS*log10(fc)
    'PL_CONST_LOS': 32.4,       # Constant term (dB), free-space reference
    'PL_COEF_DIST_LOS': 21.0,   # Distance log-coefficient (dB)
    'PL_COEF_FREQ_LOS': 20.0,   # Frequency log-coefficient (dB)
    # NLOS: PL = PL_CONST_NLOS + PL_COEF_DIST_NLOS*log10(d3D)
    #            + PL_COEF_FREQ_NLOS*log10(fc) - PL_CORRECTION_H*(h_UT - 1.5)
    'PL_CONST_NLOS': 35.3,      # NLOS constant (dB)
    'PL_COEF_DIST_NLOS': 22.4,  # NLOS distance log-coefficient (dB)
    'PL_COEF_FREQ_NLOS': 21.3,  # NLOS frequency log-coefficient (dB)
    'PL_CORRECTION_H': 0.3,     # Height correction coeff. (dB/m above 1.5m)

    # --- 3GPP LOS Probability Model (TR 38.901 Table 7.4.2-1 UMi) ---
    'LOS_D_BP': 18.0,           # Break-point distance (m): P_LOS=1 for d<=d_bp
    'LOS_DECAY_RATE': 36.0,     # Exponential decay rate (m)

    # --- Shadow Fading (3GPP Table 7.5-6 Part-1 UMi) ---
    'SHADOW_SIGMA_LOS': 4.0,    # LOS shadow fading std-dev (dB)
    'SHADOW_SIGMA_NLOS': 7.82,  # NLOS shadow fading std-dev (dB)
    'SHADOW_DCOR_LOS': 10.0,    # LOS decorrelation distance (m)
    'SHADOW_DCOR_NLOS': 13.0,   # NLOS decorrelation distance (m)

    # --- TX / Noise ---
    'PT_LIN': 0.2,              # TX power (W), ~23 dBm mmWave small cell
    'NOISE_POWER_LIN': 1e-12,   # Noise floor (W), ~-90 dBm

    # --- Hard Limits ---
    'MAX_LINK_DIST': 200.0,     # Absolute max link range (m)
    'PL_CUTOFF_DB': 140.0,      # Path-loss outage threshold (dB)

    # ========== Link-Layer / MAC: HARQ ==========
    'HARQ_MAX_RETRANS': 3,      # Max retransmissions (M). Total attempts = M+1
    'SLOT_DURATION': 1e-3,      # Slot duration (s), 1 ms for NR
...
class CoreEnv:
    ...
    def __init__(self, cfg: dict):
        ...
        def _coerce_phys(k, default):
            raw = phys_cfg.get(k, default)
            if isinstance(default, bool):
                return bool(raw)
            if isinstance(default, int) and not isinstance(default, bool):
                return int(raw)
            return float(raw)
        self._phys = {k: _coerce_phys(k, v) for k, v in DEFAULT_PHYSICS.items()}
        # Per-step link metrics cache: (u,v) -> {sinr_db, reliability, expected_delay, expected_tx}
        self._step_link_cache: Dict[Tuple[int, int], Dict[str, float]] = {}
    ...
    def step(self, deltaE: DeltaE, partitions: Dict[int, int]) -> Tuple[Dict[int, Observation], float, Dict]:
        ...
        # Build per-edge link metrics cache (SINR, BLER, expected_tx) once per step.
        # All reward components read from this cache to avoid redundant computation.
        self._build_step_link_cache()
        ...
        # Convex consensus reward: cubic amplification
        # This creates a "siphon effect" — improving from 0.8→0.9 yields much
        # more reward than 0.1→0.2, strongly motivating topology perfection.
        #   0.5 → 0.125,  0.8 → 0.512,  0.9 → 0.729,  0.95 → 0.857
        consensus_convex = float(consensus_success) ** 3

        # Combine reward with configurable weights
        reward = (
            w['consensus'] * consensus_convex
        ...
        # Step bonus for near-perfect consensus (outside multiplier so it's always felt)
        if consensus_success > 0.95:
            reward += 2.0
        elif consensus_success > 0.85:
            reward += 0.5
    ...
    def _energy_cost(self) -> float:
        """HARQ-aware energy cost.

        E_total = Σ_{(u,v)∈E}  P_TX × T_slot × E[N_trans(u,v)]

        Low-SINR links require many HARQ retransmissions → higher energy.
        Outage links (kept by agent despite infeasibility) are penalised
        with the maximum M+1 transmissions.
        """
        if len(self.nodes) <= 1:
            return 0.0

        pt_lin = float(self._phys.get('PT_LIN', 0.2))
        slot = float(self._phys.get('SLOT_DURATION', 1e-3))
        M = int(self._phys.get('HARQ_MAX_RETRANS', 3))
        max_energy = float(self._phys.get('MAX_ENERGY', 200.0))

        total_energy = 0.0
        for u, v in self.edges:
            key = tuple(sorted((u, v)))
            cached = self._step_link_cache.get(key)
            if cached is not None:
                exp_tx = cached['expected_tx']
            else:
                exp_tx = float(M + 1)
            total_energy += pt_lin * slot * exp_tx

        return float(min(1.0, total_energy / max(1e-6, max_energy)))
    ...
    # ==================================================================
    # 3GPP TR 38.901 UMi Street Canyon — PHY + MAC-Layer HARQ
    # ==================================================================

    def _compute_los_probability_3gpp(self, d_2d: float) -> float:
        """3GPP TR 38.901 Table 7.4.2-1 UMi LOS probability.

        P_LOS = 1                                                 if d_2D <= d_bp
               (d_bp/d_2D) + exp(-d_2D/decay) * (1 - d_bp/d_2D)  otherwise
        """
        d_bp = float(self._phys.get('LOS_D_BP', 18.0))
        decay_rate = float(self._phys.get('LOS_DECAY_RATE', 36.0))
        if d_2d <= d_bp:
            return 1.0
        ratio = d_bp / max(1e-6, d_2d)
        return ratio + math.exp(-d_2d / max(1e-6, decay_rate)) * (1.0 - ratio)

    def _compute_path_loss_3gpp(self, d_2d: float, is_los: bool,
                                shadow_db: float = 0.0) -> float:
        """3GPP TR 38.901 UMi Street Canyon path loss (dB).

        LOS:
          PL = α_LOS·log10(d3D) + β_LOS·log10(fc) + γ_LOS  + X_σ
        NLOS:
          PL_raw = α_NLOS·log10(d3D) + β_NLOS·log10(fc) + γ_NLOS
                   - δ_h·(h_UT - 1.5)  + X_σ
          PL = max(PL_LOS, PL_raw)   (NLOS never below co-located LOS)
        """
        fc = float(self._phys.get('FC_GHZ', 28.0))
        h_ut = float(self._phys.get('H_UT', 1.5))
        h_bs = float(self._phys.get('H_BS', 10.0))

        d_h = abs(h_bs - h_ut)
        d_3d = math.sqrt(max(1.0, d_2d) ** 2 + d_h ** 2)

        log_d3d = math.log10(d_3d)
        log_fc = math.log10(max(0.1, fc))

        # LOS path loss (always computed — needed as floor for NLOS)
        alpha_los = float(self._phys.get('PL_COEF_DIST_LOS', 21.0))
        beta_los = float(self._phys.get('PL_COEF_FREQ_LOS', 20.0))
        gamma_los = float(self._phys.get('PL_CONST_LOS', 32.4))
        pl_los = alpha_los * log_d3d + beta_los * log_fc + gamma_los

        if is_los:
            return max(0.0, pl_los + shadow_db)

        # NLOS
        alpha_nlos = float(self._phys.get('PL_COEF_DIST_NLOS', 22.4))
        beta_nlos = float(self._phys.get('PL_COEF_FREQ_NLOS', 21.3))
        gamma_nlos = float(self._phys.get('PL_CONST_NLOS', 35.3))
        delta_h = float(self._phys.get('PL_CORRECTION_H', 0.3))

        pl_nlos_raw = (alpha_nlos * log_d3d + beta_nlos * log_fc + gamma_nlos
                       - delta_h * (h_ut - 1.5))
        pl_nlos = max(pl_los, pl_nlos_raw)
        return max(0.0, pl_nlos + shadow_db)

    def _get_bler_from_sinr(self, sinr_db: float) -> float:
        """AMC waterfall curve: sigmoid approximation of BLER vs SINR.
    ...
    def _compute_link_metrics(self, sinr_db: float) -> Tuple[float, float]:
        """HARQ performance from SINR: (final_reliability, expected_delay).

        p_err          = BLER(sinr_db)
        M              = HARQ_MAX_RETRANS
        reliability    = 1 - p_err^(M+1)
        E[T]           = (1 - p_err^(M+1)) / (1 - p_err)   (truncated geometric)
        expected_delay = E[T] * SLOT_DURATION
        """
        p_err = self._get_bler_from_sinr(sinr_db)
        M = int(self._phys.get('HARQ_MAX_RETRANS', 3))
        slot = float(self._phys.get('SLOT_DURATION', 1e-3))

        total_attempts = M + 1
        if p_err >= 1.0 - 1e-12:
            return (0.0, total_attempts * slot)
        if p_err <= 1e-12:
            return (1.0, slot)

        p_fail_all = p_err ** total_attempts
        reliability = 1.0 - p_fail_all
        expected_tx = (1.0 - p_fail_all) / (1.0 - p_err)
        return (reliability, expected_tx * slot)
    ...
    def _build_step_link_cache(self) -> None:
        ...
        For every active edge, computes SINR (using local receiver-degree
        interference), BLER, reliability, expected delay, and expected
        number of HARQ transmissions.  Results are stored in
        ``self._step_link_cache`` so that ``_link_success``,
        ...
        for u, v in self.edges:
            deg[u] = deg.get(u, 0) + 1
            deg[v] = deg.get(v, 0) + 1
        self._node_degree = deg

        cache: Dict[Tuple[int, int], Dict[str, float]] = {}
        use_3gpp = bool(self._phys.get('USE_3GPP_MODEL', True))
        M = int(self._phys.get('HARQ_MAX_RETRANS', 3))
        slot = float(self._phys.get('SLOT_DURATION', 1e-3))
        ...
        for edge in self.edges:
            ...
            if use_3gpp:
                pl_db = self._link_pl_db_3gpp(u, v)

                if pl_db is None:
                    cache[key] = {
                        'sinr_db': -999.0,
                        'reliability': 1e-12,
                        'expected_delay': float(M + 1) * slot,
                        'expected_tx': float(M + 1),
                    }
                    continue

                sinr_lin_at_v = self._pl_to_sinr(pl_db, deg.get(v, 0))
                sinr_lin_at_u = self._pl_to_sinr(pl_db, deg.get(u, 0))
                bottleneck_lin = min(sinr_lin_at_v, sinr_lin_at_u)

                if bottleneck_lin <= 0.0:
                    cache[key] = {
                        'sinr_db': -999.0,
                        'reliability': 1e-12,
                        'expected_delay': float(M + 1) * slot,
                ...
                sinr_db = 10.0 * math.log10(max(1e-15, bottleneck_lin))
                rel, delay = self._compute_link_metrics(sinr_db)

                bler = self._get_bler_from_sinr(sinr_db)
                if bler >= 1.0 - 1e-12:
                    exp_tx = float(M + 1)
                elif bler <= 1e-12:
                    exp_tx = 1.0
                    ...
                    p_fail_all = bler ** (M + 1)
                    exp_tx = (1.0 - p_fail_all) / (1.0 - bler)

                cache[key] = {
                    'sinr_db': sinr_db,
                    'reliability': max(1e-12, rel),
                    'expected_delay': delay,
                    'expected_tx': exp_tx,
                }
            else:
                # Legacy model — no HARQ-aware expected_tx
                rel = self._link_success_legacy(u, v)
                delay = self._link_delay_legacy(u, v)
                snr = self._link_snr_legacy(u, v)
                snr_db = 10.0 * math.log10(max(1e-15, snr)) if snr > 0.0 else -999.0
                cache[key] = {
                    'sinr_db': snr_db,
                    'reliability': max(1e-12, rel),
                    'expected_delay': delay,
        ...
        self._step_link_cache = cache

    def _link_success(self, u: int, v: int) -> float:
        """Link reliability after HARQ, based on 3GPP UMi channel model.

        Uses the per-step cache when available; falls back to on-the-fly
        computation (e.g. during ``reset``).
        """
        key = tuple(sorted((u, v)))
        cached = self._step_link_cache.get(key)
        if cached is not None:
        ...
        if not self._phys.get('USE_3GPP_MODEL', True):
            return self._link_success_legacy(u, v)

        if u not in self.node_pos or v not in self.node_pos:
            return 0.0

        sinr_db = self._link_sinr_db_3gpp(u, v)
        if sinr_db is None:
            return 1e-12

        reliability, _ = self._compute_link_metrics(sinr_db)
        return float(max(1e-12, reliability))
    ...
    def _link_delay(self, u: int, v: int) -> float:
        """Expected one-way delay incorporating HARQ retransmissions.

        Uses the per-step cache when available.
        """
        key = tuple(sorted((u, v)))
        cached = self._step_link_cache.get(key)
        if cached is not None:
            max_delay = float(self._phys.get('MAX_DELAY', 1.0))
            return float(max(1e-6, min(max_delay, cached['expected_delay'])))
        ...
        _, expected_delay = self._compute_link_metrics(sinr_db)
        return float(max(1e-6, min(max_delay, expected_delay)))
    ...
    def _link_snr(self, u: int, v: int) -> float:
        """Linear SINR for link (u,v). Uses per-step cache when available."""
        key = tuple(sorted((u, v)))
        cached = self._step_link_cache.get(key)
        if cached is not None:
            sdb = cached['sinr_db']
            if sdb <= -900.0:
                return 0.0
            return float(max(0.0, 10.0 ** (sdb / 10.0)))
        ...
        if sinr_db is None:
            return 0.0
        return float(max(0.0, 10.0 ** (sinr_db / 10.0)))
    ...
    # ==========================================================================
    # Unified Link Model (3GPP UMi + legacy CI fallback)
    # ==========================================================================

    def _link_pl_db_3gpp(self, u: int, v: int) -> Optional[float]:
        """Compute 3GPP UMi path loss (dB) for link (u,v).

        Handles LOS probability, shadowing, and cutoff.
        Returns None if the link is in outage (exceeds distance or PL cutoff).
        """
        if u not in self.node_pos or v not in self.node_pos:
            return None
        ...
        x1, y1 = self.node_pos[u]
        x2, y2 = self.node_pos[v]
        d_2d = math.hypot(x1 - x2, y1 - y2)

        max_dist = float(self._phys.get('MAX_LINK_DIST', 200.0))
        if max_dist > 0.0 and d_2d > max_dist:
            return None

        p_los = self._compute_los_probability_3gpp(d_2d)
        key = tuple(sorted((u, v)))
        seed = hash((key, self.time_step)) & 0xFFFFFFFF
        is_los = (random.Random(seed).random() < p_los)

        shadow_db = self._get_shadowing_db(u, v, is_los)
        pl_db = self._compute_path_loss_3gpp(d_2d, is_los, shadow_db)

        cutoff = float(self._phys.get('PL_CUTOFF_DB', 140.0))
        if pl_db > cutoff:
            return None
        return pl_db

    def _link_sinr_db_3gpp(self, u: int, v: int,
                            receiver_degree: int = 0) -> Optional[float]:
        """End-to-end SINR (dB) using 3GPP UMi channel with local interference.

        Args:
            u, v: Node IDs forming the link.
            receiver_degree: In-degree of the receiving node (for local
                congestion interference).  Defaults to 0 when called
                outside of the step-level cache builder.
        """
        pl_db = self._link_pl_db_3gpp(u, v)
        if pl_db is None:
            return None

        sinr_lin = self._pl_to_sinr(pl_db, receiver_degree)
        if sinr_lin <= 0.0:
            return None
        return 10.0 * math.log10(max(1e-15, sinr_lin))
    ...
    def _get_shadowing_db(self, u: int, v: int, is_los: bool) -> float:
        ...
        """
        key = tuple(sorted((u, v)))
        # Prefer 3GPP param names, fall back to legacy
        if is_los:
            sigma = float(self._phys.get('SHADOW_SIGMA_LOS',
                          self._phys.get('PL_SIGMA_LOS', 4.0)))
        else:
            sigma = float(self._phys.get('SHADOW_SIGMA_NLOS',
                          self._phys.get('PL_SIGMA_NLOS', 7.82)))
        d_cor = float(self._phys.get('SHADOW_DCOR_LOS' if is_los else 'SHADOW_DCOR_NLOS',
                                      10.0 if is_los else 13.0))
    ...
    def _path_loss_db(self, u: int, v: int) -> Tuple[float, bool]:
        """Unified path-loss entry point used by step() edge-feasibility checks.

        Delegates to 3GPP UMi model when enabled, else legacy CI model.
        Returns (PL_dB, is_outage).
        """
        if self._phys.get('USE_3GPP_MODEL', True):
            return self._path_loss_db_3gpp(u, v)
        return self._path_loss_db_legacy_ci(u, v)

    def _path_loss_db_3gpp(self, u: int, v: int) -> Tuple[float, bool]:
        """3GPP UMi path loss with probabilistic LOS."""
        if u not in self.node_pos or v not in self.node_pos:
            cutoff = float(self._phys.get('PL_CUTOFF_DB', 140.0))
            return (cutoff + 10.0, True)
        ...
        shadow_db = self._get_shadowing_db(u, v, is_los)
        pl_db = self._compute_path_loss_3gpp(d_2d, is_los, shadow_db)
        return (pl_db, pl_db > cutoff)

    def _path_loss_db_legacy_ci(self, u: int, v: int) -> Tuple[float, bool]:
        """Legacy CI model (kept for backward compat)."""
        d_euc, is_los, d_path, n_corner = self._compute_link_geometry(u, v)
        if d_euc <= 0.0 or not math.isfinite(d_euc):
            return (float(self._phys.get('PL_CUTOFF_DB', 140.0)) + 10.0, True)
        ...
        pl_cutoff = float(self._phys.get('PL_CUTOFF_DB', 140.0))
        corner_loss = float(self._phys.get('CORNER_LOSS_DB', 6.0))
        fspl_1m = 32.4 + 20.0 * math.log10(max(1e-3, fc_ghz))
        if is_los:
            n = float(self._phys.get('PL_N_LOS', 2.2))
            pl_db = fspl_1m + 10.0 * n * math.log10(max(1.0, d_euc))
        else:
            n = float(self._phys.get('PL_N_NLOS', 2.8))
            pl_db = fspl_1m + 10.0 * n * math.log10(max(1.0, d_path)) + n_corner * corner_loss
        shadow_db = self._get_shadowing_db(u, v, is_los)
        pl_db = max(0.0, pl_db + shadow_db)
        return (pl_db, pl_db > pl_cutoff)

    def _pl_to_sinr(self, pl_db: float, receiver_degree: int = 0) -> float:
        """Convert path loss (dB) to linear SINR with local congestion interference.

        N_eff(v) = N_thermal * (1 + INTERF_K_LOCAL * Degree_in(v))

        Args:
            pl_db: Path loss in dB (symmetric for link u-v).
            receiver_degree: Current in-degree of the receiving node.
        """
        pt_lin = float(self._phys.get('PT_LIN', 0.1))
        noise_lin = float(self._phys.get('NOISE_POWER_LIN', 1e-10))

        lam = float(self._phys.get('INTERF_K_LOCAL', 0.3))
        noise_eff = noise_lin * (1.0 + lam * float(receiver_degree))

        gain_lin = 10.0 ** (-pl_db / 10.0)
        sinr = (pt_lin * gain_lin) / max(1e-15, noise_eff)
        return max(0.0, sinr)
    ...
    def compute_global_context(self) -> List[float]:
        ...
        interf_k = float(self._phys.get('INTERF_K_LOCAL', 0.0))
        norm_interf_k = min(1.0, interf_k / 1.0)
