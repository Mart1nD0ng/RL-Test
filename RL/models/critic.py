...
class Critic(nn.Module):
    """Set Transformer Critic for CTDE with optional Global Context.

    Input:
      X: Set of agent embeddings [Batch, m, F]
      context (optional): Global context vector [Batch, context_dim]

    Output: Global Value V(s)

    When context_dim > 0, the context is projected to hidden_dim and
    additively fused with the pooled agent representation before the
    final value head. This lets the Critic "see" macro-level features
    (edge density, LCC ratio, interference level, etc.) that are not
    visible in any single agent's local observation.
    """

    def __init__(self, input_dim: int = 128, hidden_dim: int = 128, output_dim: int = 1,
                 num_heads: int = 4, num_inds: int = 16, context_dim: int = 0):
        ...
        self._pma = PMA(hidden_dim, num_heads, 1, ln=True)
        ...
        self._value_head = nn.Linear(hidden_dim, output_dim)

        # Backward-compatible alias: PopArtCritic accesses dec[-1] for
        # weight rescaling. We expose the value head at that position.
        self.dec = nn.ModuleList([self._pma, self._value_head])

    def forward(self, X: torch.Tensor, context: torch.Tensor = None) -> torch.Tensor:
        h = self.enc(X)
        pooled = self._pma(h).squeeze(1)  # [batch, hidden_dim]

        if context is not None and self.context_proj is not None:
            ctx = self.context_proj(context)  # [batch, hidden_dim]
            pooled = pooled + ctx

        out = self._value_head(pooled).squeeze(-1)  # [batch]
        return out
...
class PopArtCritic(nn.Module):
    ...
    def forward(self, X: torch.Tensor, context: torch.Tensor = None) -> torch.Tensor:
        ...
        v_normalized = self.critic(X, context=context)
        v_actual = self.sigma * v_normalized + self.mu
        return v_actual

    def forward_normalized(self, X: torch.Tensor, context: torch.Tensor = None) -> torch.Tensor:
        """前向传播，返回归一化的预测值（用于训练时与归一化目标对比）"""
        return self.critic(X, context=context)
