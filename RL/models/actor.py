...
class SimpleGCNLayer(nn.Module):
    """GCN layer with mask-aware neighbour aggregation.

    Supports variable-size graphs by ignoring zero-padded slots via *mask*.
    When *mask* is provided the degree normalisation only counts valid
    neighbours and the output for invalid slots is zeroed out, so that
    downstream pooling is unaffected by padding size.
    """
    ...
    def forward(self, x: torch.Tensor, adj: torch.Tensor,
        ...
        Args:
            ...
            adj:  [N, N]  adjacency (symmetric, with self-loops)
            mask: [N]     float mask (1.0 = valid, 0.0 = padded), optional
        """
        if mask is not None:
            # Zero out rows/cols of padded nodes so they neither
            # send nor receive messages.
        ...
        if mask is not None:
            h = h * mask.unsqueeze(-1)  # zero out padded slot features
        return h
...
class Actor(nn.Module):
    ...
    def _slot_mask(self, obs: Observation) -> torch.Tensor:
        """Return a float mask [slots] where 1.0 = valid node, 0.0 = padding."""
        masks: List[float] = []
        for s in obs.slots[: self.slots]:
    ...
    def _post_embed(self, obs: Observation) -> torch.Tensor:
        ...
        # Build validity mask from slot masks
        mask = self._slot_mask(obs)                         # [slots]

        h1 = F.relu(self.gcn1(x, A, mask))
        h2 = F.relu(self.gcn2(h1, A, mask))

        # Masked mean pooling – only average over valid (non-padded) slots
        n_valid = mask.sum().clamp(min=1.0)
        pooled = (h2 * mask.unsqueeze(-1)).sum(dim=0) / n_valid
