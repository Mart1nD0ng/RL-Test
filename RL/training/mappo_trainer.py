...
class Transition:
    ...
    global_context: torch.Tensor  # [1, context_dim] for Critic awareness
    idx: int
    op: int
    cands: List[Tuple[int, int]]
    internal_cands: List[Tuple[int, int]]
    cross_cands: List[Tuple[int, int]]
    logprob: float
    value: float
    reward: float
    done: bool
...
class MAPPOTrainer:
    ...
    def store_transition(self, agent_id: int, obs, global_state, idx: int, op: int, cands: List[Tuple[int, int]],
        ...
        if internal_cands is None:
            internal_cands = cands
        if cross_cands is None:
            cross_cands = []
        if global_context is None:
            global_context = torch.zeros(1, 6)
        ...
        self.buffer.append(Transition(agent_id, obs, global_state, global_context, idx, op, cands, internal_cands, cross_cands, logprob, value, reward, done))
    ...
    def update(self):
        ...
        # PPO epochs with MINIBATCH + per-minibatch KL gate
        #
        # Key fix: the old code only checked KL at epoch boundaries.
        # With 20 minibatch steps per epoch, KL ballooned to ~0.6 (20x target)
        # before the check fired.  Now we compute an approximate KL after EVERY
        # minibatch step using the PPO-standard formula:
        #
        #   approx_kl = E[ (ratio - 1) - log(ratio) ]       (always >= 0)
        #
        # If it exceeds the trust-region threshold we break IMMEDIATELY,
        # keeping the policy close to the old one.
        observed_kl = 0.0
        lr_changed = False
            ...
            for mb_start in range(0, n_samples, mb):
                ...
                for local_idx in mb_indices:
                    ...
                    if hasattr(self.critic, 'forward_normalized'):
                        val_pred = self.critic.forward_normalized(tr.global_state, context=tr.global_context)
                    else:
                        val_pred = self.critic(tr.global_state, context=tr.global_context)
                ...
                if total_grad_steps > 0 and self.target_kl and observed_kl > 0:
                ...
                # --- Per-minibatch approximate KL gate ---
                # Standard PPO KL approximation: E[(r-1) - log(r)]
                # This is always non-negative and equals the true KL
                # to second order.  Much cheaper than a full-buffer pass.
                with torch.no_grad():
                    log_ratio = new_logps - mb_old_logps
                    approx_kl = float(((ratio - 1) - log_ratio).mean().item())
                    observed_kl = max(0.0, approx_kl)
                ...
                if self.target_kl and observed_kl > 1.5 * self.target_kl:
                    if self.adapt_lr_on_kl:
                        ...
                        if lr_changed:
                            try:
                                print(f"[KL Gate] ep/mb {epoch}/{mb_start//mb} "
                                      f"KL={observed_kl:.5f} > {1.5*self.target_kl:.5f}; "
                                      f"steps={total_grad_steps}; lr={self._last_lr:.2e}")
                            except Exception:
                                pass
                    kl_break = True
                    break

        # Final KL: use last observed approximate KL (already computed above)
        kl = observed_kl
