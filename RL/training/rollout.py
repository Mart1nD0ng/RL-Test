...
def build_observations(env: CoreEnv, partitions: Dict[int, int], bus: MessageBus, role_ids: Dict[int, int], cfg: dict):
    ...
    Notes:
        - Slot padding to ``model.slots`` is handled inside
          ``env._build_observations`` (zero-padded slots with ``mask=0``).
        - The Actor uses each slot's ``mask`` field as a validity indicator
          for masked GCN aggregation and pooling, so no separate mask tensor
