...
def run(cfg_path: str | None = None, overrides: dict | None = None):
    ...
    for ep in range(episodes):
        ...
        for t in range(steps_T):
            ...
            # BUG修复：使用 Critic 获取 value（而不是 Actor 的未训练 value head）
            # 这确保 GAE 计算使用的是与 PPO 更新一致的值估计
            with torch.no_grad():
                critic_value = critic(global_state_t).item()  # Centralized value
            
            for aid, obs in obs_by_agent.items():
                act, aux = actor.act(obs)
                # 覆盖 Actor 的 value 为 Critic 的 centralized value
